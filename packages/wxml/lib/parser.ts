...
export class Parser implements Callbacks {
  ...
  private readonly callbacks: Partial<Handler>
  ...
  constructor(
    callbacks?: Partial<Handler> | null,
    ...
    this.callbacks.onParserInit?.(this)
  }
  ...
  onTextEntity(cp: number, endIndex: number): void {
    this.endIndex = endIndex - 1
    this.callbacks.onText?.(fromCodePoint(cp))
    this.startIndex = endIndex
  }
  ...
  private emitOpenTag(name: string) {
    ...
    if (impliesClose) {
      while (this.stack.length > 0 && impliesClose.has(this.stack[0])) {
        ...
        this.callbacks.onCloseTag?.(element, true)
      }
    }
    ...
    this.callbacks.onOpenTagName?.(name)
    if (this.callbacks.onOpenTag) this.attribs = {}
  }
  ...
  private endOpenTag(isImplied: boolean) {
    ...
    if (this.attribs) {
      this.callbacks.onOpenTag?.(this.tagname, this.attribs, isImplied)
      this.attribs = null
    }
    if (this.callbacks.onCloseTag && this.isVoidElement(this.tagname)) {
      this.callbacks.onCloseTag(this.tagname, true)
    }
    ...
    if (!this.isVoidElement(name)) {
      ...
      if (pos !== -1) {
        for (let index = 0; index <= pos; index++) {
          ...
          this.callbacks.onCloseTag?.(element, index !== pos)
        }
      ...
      this.callbacks.onOpenTagName?.('br')
      this.callbacks.onOpenTag?.('br', {}, true)
      this.callbacks.onCloseTag?.('br', false)
    }
  ...
  private closeCurrentTag(isOpenImplied: boolean) {
    ...
    if (this.stack[0] === name) {
      ...
      this.callbacks.onCloseTag?.(name, !isOpenImplied)
      this.stack.shift()
    }
  }

  /** @internal */
  onAttributeName(start: number, endIndex: number): void {
  ...
  /** @internal */
  onAttributeData(start: number, endIndex: number): void {
    this.attribvalue += this.getSlice(start, endIndex)
  }

  /** @internal */
  onAttributeEntity(cp: number): void {
    this.attribvalue += fromCodePoint(cp)
  }

  /** @internal */
  onAttributeEnd(quote: QuoteType, endIndex: number): void {
    this.endIndex = endIndex
  ...
  onDeclaration(start: number, endIndex: number): void {
    ...
    if (this.callbacks.onProcessingInstruction) {
      const name = this.getInstructionName(value)
      this.callbacks.onProcessingInstruction(`!${name}`, `!${value}`)
    }
  ...
  onComment(start: number, endIndex: number, offset: number): void {
    ...
    this.callbacks.onComment?.(this.getSlice(start, endIndex - offset))
    this.callbacks.onCommentEnd?.()
  ...
  onCData(start: number, endIndex: number, offset: number): void {
    ...
    if (!this.htmlMode || this.options.recognizeCDATA) {
      this.callbacks.onCDataStart?.()
      this.callbacks.onText?.(value)
      this.callbacks.onCDataEnd?.()
    } else {
      this.callbacks.onComment?.(`[CDATA[${value}]]`)
      this.callbacks.onCommentEnd?.()
    }
  ...
  /** @internal */
  onEnd(): void {
    if (this.callbacks.onCloseTag) {
      // Set the end index for all remaining tags
      this.endIndex = this.startIndex
      for (let index = 0; index < this.stack.length; index++) {
        this.callbacks.onCloseTag(this.stack[index], true)
      }
    }
  ...
  public write(chunk: string): void {
    if (this.ended) {
      this.callbacks.onError?.(new Error('.write() after done!'))
      return
    }
  ...
  public end(chunk?: string): void {
    if (this.ended) {
      this.callbacks.onError?.(new Error('.end() after done!'))
      return
    }
